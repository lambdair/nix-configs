...
No reference image is committed; `flower-theme/sources/` records the image
URLs and the sampled boxes.
...
- `flowerTheme.mode` — `"auto"` (default) follows the appearance as described
  below; `"dark"` or `"light"` gives every app and the prompt that variant
...
- A starship `custom` module prints the flower's symbol and name, which Nix
  ...
  an appearance change. With `flowerTheme.mode` pinned, the pinned polarity is
  printed instead.
...
1. `lib.nix`.
2. `check.bb` and the `checks` flake output.
3. `anemone.toml`.
4. `nemophila.toml`.
5. `sunflower.toml`.
6. Overview page.
7. Module with the `flower` option, selected in `home/default.nix`.
8. The `mode` option.

Revisions 1–8 are the mechanism and apply no theme to any app; the ports
build on them:
...
11. kitty port.
12. WezTerm port.
13. Prompt indicator: the starship module.
14. The ports follow the `mode` option.
