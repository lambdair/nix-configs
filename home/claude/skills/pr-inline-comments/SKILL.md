...
インラインコメント向きの典型:
- なぜここを変えたか。そのうえで、なぜこの分岐 / この実装方法を選んだか（他の自明な選択肢を採らなかった理由）
...
hunk を見たレビュアーが最初に持つ疑問は「なぜここを変えたのか」。コメントはこの順で書く:

1. **1文目: その変更をしなかったら何が困るか。** なぜ丸める場所を移したか、なぜこの関数を作ったか、なぜ組み立て直したか、なぜこの API を呼ぶ必要があるか、このテストが何を確かめるか
2. **2文目以降: 二次的な選択。** なぜ A でなく B の方法か、なぜこの置き場所か、差分をどう読むか

方法の選択や置き場所だけを書いたコメントは、読み終えても「で、なぜ変えたの?」が残る。

- 方法の選択だけ:
  > `Math/round` でなく `BigDecimal/valueOf` なのは、それまでの `%.2f` と同じ `1.01` にするため。
- 変更の理由が先:
  > 保存に使う数値を画面と同じにするため、丸めを文字列にする前に移した。丸めていない値は、どこで文字列にするかで桁が変わる。たとえば `1.005` は Java の `%.2f` だと `1.01`、JS の `toFixed(2)` だと `1.00` になる。
  >
  > `Math/round` でなく `BigDecimal/valueOf` なのは、それまでの `%.2f` と同じ `1.01` にするため。
...
- その変更をなぜしたか。1文目に置く
- なぜ他の自明な選択肢を採らなかったか
- どういう条件で壊れるか、何を変えたら見直しが要るか。条件は名指しする。「条件に関係なく取りに行かない」ではなく「0行・マッピング無し・上限超過のときに取りに行かない」
...
3. 各コメントの1文目が、その変更をなぜしたかに答えているか確かめる（「1文目で、その変更をなぜしたかに答える」参照）
4. 各コメントについて、コードとソースコメントを読んで分かることを削る（「短く書く」参照）
5. owner / repo / PR番号 / head SHA を取得する
6. 1件なら単一コメント、複数なら review として一括投稿する（コミットごとに紐づけるなら「コミット単位で紐づける」）
7. 投稿結果の `.line` が `null` でないことを確認し、PR の URL を報告する
