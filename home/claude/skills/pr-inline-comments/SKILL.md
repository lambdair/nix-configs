...
| **コミットメッセージ** | そのリビジョンの変更前の問題と、何を変えたか |
| **インラインコメント** | 行・hunk 固有の rationale。「コードを読んだ人がここで疑問を持つだろう」という箇所に、その場で答える |

**判断基準**: その説明が特定の行を指しているなら → インラインコメント。そのリビジョン単体で閉じるなら → コミットメッセージ。複数リビジョンにまたがる方針なら → 本文。本文に「`foo.clj:42` の分岐は〜」のように行を名指しする説明を書きそうになったら、それはインラインコメントにすべきサイン。
...
**添字・順序・オフセットの説明には例を1つ付ける。** 文だけでは伝わらない。値は既存のテストケースから取る。作り話にならず、レビュアーがそのテストへ飛べる。

## PR の外を指すときは permalink を貼る

根拠は **公式ドキュメント → 実装** の順で探す。ドキュメントに書かれていないことを実装から示すときは、「ドキュメントには無い」と明記してから実装を引く。書かないと、読み手が公式を探して見つからず迷う。
...
5. 1件なら単一コメント、複数なら review として一括投稿する（コミットごとに紐づけるなら「コミット単位で紐づける」）
6. 投稿結果の `.line` が `null` でないことを確認し、PR の URL を報告する
...
スタックした PR で「コミットごとに読む」レビューをしてもらうなら、各コメントをそのコミットに紐づける。**review エンドポイントは `comments[]` ごとに `commit_id` を指定できない**ので、単一コメントのエンドポイントを1件ずつ叩く。

```bash
# commit_id を含む JSON を1件ずつ。レスポンスの line を必ず見る
gh api repos/<owner>/<repo>/pulls/<PR番号>/comments --input c0.json \
  --jq '"\(.path):\(.line)"'
```

- `line` / `start_line` は **そのコミット時点のファイルの行番号**。head の行番号ではない（`jj file show -r <rev> <path> | cat -n` で数える）
- 紐づけ先は「その変更を入れたコミット」ではなく **その行を最後に触ったコミット**。後のコミットが書き換えた行に貼ると outdated になる
- **レスポンスの `.line` が `null` なら outdated。** 投稿のたびに確認し、null なら削除して行が生きているコミットに貼り直す
- `original_commit_id` が指定したコミット、`commit_id` は「その行がまだ有効な最新のコミット」に GitHub が自動更新する。後者が head になっていても紐づけは失われていない

## 貼ったあとの手入れ

- **説明対象のコードを消したら、コメントも消す。** 別方式に載せ替えて主張が逆になった場合も、その場で書き直す。古い rationale は無いより悪い
- **force push ではコメントは消えない。** 行の内容が変わっていなければ GitHub が新しいコミットへ付け替える。行ごと消した／書き換えた場合だけ outdated になる。force push の前に「コメントが失われる」と説明しないこと
- 一覧と outdated の確認:

```bash
gh api repos/<owner>/<repo>/pulls/<PR番号>/comments \
  --jq '.[] | "\(.original_commit_id[0:8]) \(.path):\(.line) \(.body[0:30])"'
```

## 注意

- **「X を使わなかった理由」は、X を実際に確かめてから書く。** 相手の API や実装を見ずに「X ではできない」と書くと、レビューで覆る。確かめた結果、残る理由が薄いなら書かない（薄い理由は、無いより悪い）
